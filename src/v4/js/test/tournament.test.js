...
const Tournament_1 = require("../lib/Tournament");
const Initializer_1 = __importDefault(require("../lib/util/Initializer"));
const testData = __importStar(require("./data/tournament.testData"));
let tournament1, tournament2, tournament3;
...
describe('smashgg Tournament', function () {
    ...
    before(async function () {
        ...
        let ti1 = await Tournament_1.Tournament.getById(TOURNAMENT_ID_1);
        let ti2 = await Tournament_1.Tournament.getById(TOURNAMENT_ID_2);
        let ti3 = await Tournament_1.Tournament.getById(TOURNAMENT_ID_3);
        let ts1 = await Tournament_1.Tournament.get(TOURNAMENT_SLUG_1);
        let ts2 = await Tournament_1.Tournament.get(TOURNAMENT_SLUG_2);
        let ts3 = await Tournament_1.Tournament.get(TOURNAMENT_SLUG_3);
    ...
    it('should correctly search attendees and find a match', async function () {
        const searched = await tournament1.searchAttendees('GAwes');
    ...
    it('should correctly search attendees and find no match', async function () {
        const searched = await tournament1.searchAttendees('GAwes2');
        expect(searched).to.be.null;
    });
    it('should correctly search attendees and find multiple matches', async function () {
        const searched = await tournament1.searchAttendees('GA');
        expect(searched).to.not.be.null;
        expect(searched.length).to.be.greaterThan(0);
        expect(searched[0].getGamerTag()).to.be.equal('GAwes');
    ...
    it('should correctly search attendees by prefix and find no match', async function () {
        const searched = await tournament1.searchAttendeesBySponsorTag('faketaglol');
        expect(searched).to.be.null;
    });
    it('should lower case an upper case prefix on sponsor tag search', async function () {
        const searched = await tournament1.searchAttendeesBySponsorTag('rcs');
        expect(searched).to.not.be.null;
        expect(searched.length).to.be.greaterThan(0);
    ...
    it('should correctly search attendees by prefix and find more than one match', async function () {
        const searched = await tournament1.searchAttendeesBySponsorTag('ss');
        expect(searched).to.not.be.null;
        expect(searched.length).to.be.greaterThan(0);
